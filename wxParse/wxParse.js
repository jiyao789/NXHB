...
function wxParseImgTap(e) {
  ...
  if (typeof (tagFrom) != 'undefined' && tagFrom.length > 0) {
    var tempUrls = that.data[tagFrom].imageUrls;
    for (var index in tempUrls) {
      // console.log(tempUrls[index]);
    }
    /*var tempUrls = that.data[tagFrom].imageUrls;
    var newarray = [];
    for (var index in tempUrls) {
    ...
    if (nowImgUrl.indexOf('https') < 0) {
      nowImgUrl = nowImgUrl.replace('http', 'https');
    }*/
...
function wxAutoImageCal(originalWidth, originalHeight, that, bindName, temImage) {
  ...
  var arr = temImage.attr.style.join("");
