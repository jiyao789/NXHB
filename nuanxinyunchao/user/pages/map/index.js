...
Page({
    ...
    markertap(e) {
        ...
        const markerId = e.detail && e.detail.markerId !== undefined && e.detail.markerId !== null
            ? e.detail.markerId
            : e.markerId;
        this.navigateToDetail(markerId);
    },
    callouttap(e) {
        const markerId =
    ...
    navigateToDetail(markerDisplayId) {
        const markers = this.data.displayMarkers;
        const target = markers.find((m) => m.id === markerDisplayId);
        if (!target || !target.customData || !target.customData.backendId) {
            return;
        }
        const iconPath = target.customData.icon || "";
        const title = encodeURIComponent(target.customData.title || "");
        const bid = encodeURIComponent(target.customData.backendId);
        const lat = target.latitude != null ? target.latitude : DEFAULT_LAT;
        const lng = target.longitude != null ? target.longitude : DEFAULT_LNG;
        wx.navigateTo({
            url: `/nuanxinyunchao/user/pages-sub/map/detail?id=${bid}&title=${title}&icon=${encodeURIComponent(iconPath)}&lat=${lat}&lng=${lng}`
        });
    }
});
