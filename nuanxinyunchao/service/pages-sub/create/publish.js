import { publishContent, addOrUpdateDraft, deleteDraft } from '../../api/creator.js';
import { getBoundPartyCenters } from '../../api/auth.js';
...
Page({
  ...
  handlePreview() {
    ...
    if (isActivity) {
      ...
      wx.navigateTo({
        url: `./mode/surroundings?isPreview=true&data=${encodeURIComponent(JSON.stringify(paramData))}`
      });
      return;
    }
    ...
    if (this.data.isNeedYou) {
      ...
      wx.navigateTo({
        url: `/nuanxinyunchao/service/pages-sub/create/mode/needyou?isPreview=true&data=${encodeURIComponent(JSON.stringify(paramData))}`
      });
      return;
    }
    ...
    if (isLearning) {
      ...
      wx.navigateTo({
        url: '/nuanxinyunchao/service/pages-sub/create/mode/study?isPreview=true',
        events: {
    ...
    if (isOfficial) {
      ...
      wx.setStorageSync('previewData', previewData);
      wx.navigateTo({
        url: '/nuanxinyunchao/service/pages-sub/message/detail?isPreview=true&isOfficial=true',
        events: {
          submitFromPreview: () => { this.handleSubmit(); }
        }
      });
  ...
  async loadPartyCenters() {
    ...
    try {
      const res = await getBoundPartyCenters();
