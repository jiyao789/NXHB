/**
 * weapp-qrcode - 微信小程序二维码生成工具 (Canvas 2D 版)
 * 轻量级纯 JS 实现，支持 Canvas 2D API
 * 
 * 使用方法:
 *   import drawQrcode from './weapp-qrcode.js';
 *   drawQrcode({ canvas, width: 200, text: 'hello', padding: 10 });
 */
...
const QRUtil = {
  ...
  getLengthInBits(mode, type) {
    if (1 <= type && type < 10) {
      switch (mode) { case QRMode.MODE_NUMBER: return 10; case QRMode.MODE_ALPHA_NUM: return 9; case QRMode.MODE_8BIT_BYTE: return 8; default: throw new Error("mode:" + mode); }
    } else if (type < 27) {
      switch (mode) { case QRMode.MODE_NUMBER: return 12; case QRMode.MODE_ALPHA_NUM: return 11; case QRMode.MODE_8BIT_BYTE: return 16; default: throw new Error("mode:" + mode); }
    } else if (type < 41) {
      switch (mode) { case QRMode.MODE_NUMBER: return 14; case QRMode.MODE_ALPHA_NUM: return 13; case QRMode.MODE_8BIT_BYTE: return 16; default: throw new Error("mode:" + mode); }
...
function getTypeNumber(text, errorCorrectLevel) {
  const utf8 = utf16to8(text);
  const mode = QRMode.MODE_8BIT_BYTE;
  for (let typeNumber = 1; typeNumber <= 40; typeNumber++) {
    const rsBlocks = QRRSBlock.getRSBlocks(typeNumber, errorCorrectLevel);
    let totalDataCount = 0;
    for (let i = 0; i < rsBlocks.length; i++) totalDataCount += rsBlocks[i].dataCount;
    const lengthBits = QRUtil.getLengthInBits(mode, typeNumber);
    const dataBits = 4 + lengthBits + utf8.length * 8;
    if (dataBits <= totalDataCount * 8) return typeNumber;
  }
...
function drawQrcode(options) {
  const { canvas, width, padding = 0, text, background = '#ffffff', foreground = '#000000', callback } = options;
  if (!canvas || !text) return;

  const errorCorrectLevel = QRErrorCorrectLevel.M;
  const typeNumber = getTypeNumber(text, errorCorrectLevel);
  ...
  const tileW = drawSize / moduleCount;
  const tileH = drawSize / moduleCount;
  ...
  for (let row = 0; row < moduleCount; row++) {
    for (let col = 0; col < moduleCount; col++) {
      ctx.fillStyle = qr.isDark(row, col) ? foreground : background;
      const x = Math.round(col * tileW + padding);
      const y = Math.round(row * tileH + padding);
